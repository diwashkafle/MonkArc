// components/ProtectedUiComponents/journeys/github/repo-selector.tsx

'use client'

import { useState, useEffect } from 'react'
import { Lock, Globe, Loader2, AlertCircle, Check, ChevronDown, Search, Settings, ExternalLink } from 'lucide-react'
    ...
    return (
      <div className="text-sm text-amber-600 bg-amber-50 border border-amber-200 rounded-lg p-4">
        ...
        <p className="text-xs mb-3">
          Make sure you granted MonkArc access to at least one repository during installation.
        </p>
        {installationId && (
          <a
          ...
          <div className="flex items-center gap-2 flex-1 min-w-0">
            {selectedRepoData ? (
              <>
                ...
                <span className="truncate text-slate-900">{selectedRepoData.fullName}</span>
                {selectedRepoData.language && (
                  <span className="text-xs text-slate-500">• {selectedRepoData.language}</span>
                )}
              </>
            ) : (
              <span className="text-slate-500">Select a repository ({repos.length} available)</span>
            )}
          </div>
